---@meta PixelEngine API

---@class Time 
Time = {} 

---@return number 
function Time.GetDeltaTime() end 

---@return number 
function Time.GetTotalTime() end 

---@return number 
function Time.GetFPS() end 

---@class Setting 
Setting = {} 

---@param SceneName string 
---@return void 
function Setting.ChangeScene(SceneName) end 

---@param LuaPath string 
---@return void 
function Setting.CreateScene(LuaPath) end 

---@class Input 
Input = {} 

---@param key number 
---@return boolean 
function Input.GetKey(key) end 

---@param key number 
---@return boolean 
function Input.GetKeyDown(key) end 

---@param key number 
---@return boolean 
function Input.GetKeyUp(key) end 

---@return number 
function Input.GetMousePosition_X() end 

---@return number 
function Input.GetMousePosition_Y() end 

---@class Engine 
Engine = {} 

---@param name? string 
---@return GameObject 
function Engine.CreateGameObject(name) end 

---@param texturePath string 
---@return void 
function Engine.LoadTexture(texturePath) end 

---@param FindName string 
---@return GameObject 
function Engine.Find(FindName) end 

---@class GameObject 
GameObject = {} 

---@generic T 
---@param moduleName `T` 
---@return T 
function GameObject:GetModule(moduleName) end 

---@generic T 
---@param moduleName `T` 
---@return T 
function GameObject:AddModule(moduleName) end 

---@return void 
function GameObject:Destroy() end 

---@enum KeyCode
KeyCode = {
    LButton = 0x01, RButton = 0x02, Cancel = 0x03, MButton = 0x04, 
    Backspace = 0x08, Tab = 0x09, Clear = 0x0C, Enter = 0x0D, 
    Shift = 0x10, Control = 0x11, Alt = 0x12, Pause = 0x13, CapsLock = 0x14, 
    Escape = 0x1B, Space = 0x20, PageUp = 0x21, PageDown = 0x22, End = 0x23, Home = 0x24, 
    Left = 0x25, Up = 0x26, Right = 0x27, Down = 0x28, 
    Select = 0x29, Print = 0x2A, Execute = 0x2B, PrintScreen = 0x2C, Insert = 0x2D, Delete = 0x2E, 
    Alpha0 = 0x30, Alpha1 = 0x31, Alpha2 = 0x32, Alpha3 = 0x33, Alpha4 = 0x34, 
    Alpha5 = 0x35, Alpha6 = 0x36, Alpha7 = 0x37, Alpha8 = 0x38, Alpha9 = 0x39, 
    A = 0x41, B = 0x42, C = 0x43, D = 0x44, E = 0x45, F = 0x46, G = 0x47, H = 0x48, 
    I = 0x49, J = 0x4A, K = 0x4B, L = 0x4C, M = 0x4D, N = 0x4E, O = 0x4F, P = 0x50, 
    Q = 0x51, R = 0x52, S = 0x53, T = 0x54, U = 0x55, V = 0x56, W = 0x57, X = 0x58, 
    Y = 0x59, Z = 0x5A, 
    Numpad0 = 0x60, Numpad1 = 0x61, Numpad2 = 0x62, Numpad3 = 0x63, Numpad4 = 0x64, 
    Numpad5 = 0x65, Numpad6 = 0x66, Numpad7 = 0x67, Numpad8 = 0x68, Numpad9 = 0x69, 
    Multiply = 0x6A, Add = 0x6B, Separator = 0x6C, Subtract = 0x6D, Decimal = 0x6E, Divide = 0x6F, 
    F1 = 0x70, F2 = 0x71, F3 = 0x72, F4 = 0x73, F5 = 0x74, F6 = 0x75, 
    F7 = 0x76, F8 = 0x77, F9 = 0x78, F10 = 0x79, F11 = 0x7A, F12 = 0x7B, 
}

